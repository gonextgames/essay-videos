...
export default makeScene2D(function* (view) {
  ...
  yield* panes.terminalContentsRef().edit(2/8, false)`UserComputer:projects User$ ${insert(`templative`)}`

  yield* waitUntil('showProduce');
  yield* panes.terminalContentsRef().edit(2/8, false)`UserComputer:projects User$ templative${insert(` produce`)}`;
...
`v projects\n\tv potionShmotion\n\t\t> art\n\t\t> artdata\n\t\t> gamedata${insert(`\n\t\t> output`)}\n\t\tcomponent-compose.json\n\t\tgame-compose.json\n\t\tgame.json\n\t\trules.md\n\t\tstudio.json`;
  ...
  yield* all(
    ...
    yield panes.terminalContentsRef().edit(2/8, false)`UserComputer:projects User$ ${remove(`templative produce`)}`
  )
